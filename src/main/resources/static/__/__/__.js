var __ = {
	...
	api : function(method, url, data, callback) {
		...
		var config = {
			headers : {
				...
				'Expires' : '0',
				'If-Modified-Since' : '0',
				'Access-Token' : __.getAccessToken()
			},
			method : method,
			url : __.url(url)
		};
	...
	toPagination : function(currentPage, limit) {
		!limit && (limit = 15);
		return {
			current : currentPage,
			offset : Math.max((currentPage | 0) - 1, 0) * (limit | 0),
			limit : limit,
			filter : {}
		};
	},
...
(function() {
	var vmLoading = false, maskHash = {}, MyDialog = Vue.extend({
		...
		' <iframe :name="uuid" :src="url" :height="height" style="margin: 0 0;padding: 0 0;width: 100%;border: none;"></iframe>',//
		'</div>' ].join(''),
		data : function() {
			return {
				visible : false,
		...
		methods : {
			handleOpen : function() {
				var me = this;
				var fullscreen = me.fullscreen || false;
				if (fullscreen) {
					me.$nextTick(function() {
						var el = me.$el;
						var elBody = el.querySelector('.el-dialog__body');
						elBody.style.height = 'calc(100% - 120px)';
						elBody.querySelector('iframe').style.height = '100%';
					});
				}
			},
			...
			handleCloled : function() {
				...
				__.defer(function() {
					...
					delete __.dialog.store[me.uuid];
				}, 1000);
			}
		}
	...
	__.extend(__, {
		...
		dialog : {
			...
			open : function(options) {
				var _opener_ = __.top();
				if (_opener_ != __ && options.scope !== 'self') {
				...
				if (options.fullscreen == true) {
					...
					cfg.fullscreen = true;
					cfg.height = '100%'
					// Math.max(window.innerHeight - 123, 255) + 'px'
					cfg.fullscreen = true;
				}
